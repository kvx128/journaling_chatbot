...
from services.orchestrator.lifecycle.bot_lifecycle import BotLifecycle
...
def bots_status() -> dict[str, dict[str, bool]]:
    fc = FinanceClient()
    jc = JournalClient()
    ...
    return {
        "finance_bot": {
            "container_running": lifecycle.is_running("journaling-finance-bot"),
            "api_ready": fc.is_ready(),
        },
        "journal_bot": {
            "container_running": lifecycle.is_running("journaling-journal-bot"),
            "api_ready": jc.is_ready(),
        },
    }
