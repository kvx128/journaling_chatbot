...
from shared.models.orm import Transaction, User
from shared.models.schemas import FinanceSummaryResponse, TransactionCreate
from shared.repositories.transaction_repo import TransactionRepository


def log_transaction_from_text(db: Session, user: User, text: str) -> tuple[str, Transaction | None]:
...
def create_transaction_manual(db: Session, user: User, txn_create: TransactionCreate) -> Transaction:
    repo = TransactionRepository(db)
    return repo.create(
        user_id=user.id,
        amount_minor=txn_create.amount_minor,
        category=txn_create.category,
        direction=txn_create.direction,
        merchant=txn_create.merchant,
        occurred_on=txn_create.occurred_on,
        payment_method=txn_create.payment_method,
        source=txn_create.source,
        raw_text=txn_create.raw_text,
        extraction_confidence=None,
        confirmed=txn_create.confirmed,
    )
...
def get_summary(db: Session, user: User, date_range: DateRangeEnum, category: Category | None = None) -> FinanceSummaryResponse:
    start_date, end_date = _get_dates_for_range(date_range)
    repo = TransactionRepository(db)
    summary_data = repo.summarize(user.id, start_date, end_date, category)
...
def answer_finance_query(db: Session, user: User, text: str) -> str:
    ...
    summary = get_summary(db, user, date_range, category)
